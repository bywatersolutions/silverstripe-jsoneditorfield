JSONEditor.defaults.themes.silverstripe = JSONEditor.AbstractTheme.extend({
  ...
  afterInputReady: function(input) {
    ...
    if (this.closest(input, ".compact")) {
      ...
      input.controlgroup.style.marginLeft = 0;
      input.controlgroup.style.marginRight= 0;
    }
  ...
  getIndentedPanel: function() {
    ...
    el.classList.add('card-body', 'bg-light');
    return el;
  },
  ...
  getButton: function(text, icon, title) {
    ...
    if (text.startsWith('Add')) {
       el.classList.add("btn", "btn-primary", "font-icon-plus-circled");
    } else if (text.startsWith('Delete')) {
       el.classList.add("btn", "btn-danger", "font-icon-trash-bin");
    } else if (text == 'Object Properties') {
       el.classList.add("btn", "btn-primary", "font-icon-menu");
    } else if (text == 'Move up') {
       el.classList.add("btn", "btn-info", "font-icon-up-circled");
    } else if (text == 'Move down') {
       el.classList.add("btn", "btn-info", "font-icon-down-circled");
    } else if (text == 'Edit JSON') {
       el.classList.add("btn", "btn-dark", "font-icon-pencil");
    }
    return el;
  },
  getTable: function() {
  ...
  getTopTabHolder: function(propertyName) {
    var pName = (typeof propertyName === 'undefined')? "" : propertyName;
    var el = document.createElement('div');
    el.innerHTML = "<ul class='nav nav-tabs' id='" + pName + "'></ul><div class='tab-content' id='" + pName + "'></div>";
    return el;
  },
